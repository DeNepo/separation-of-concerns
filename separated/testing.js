...
export const testing = (func, tests) => {
  ...
  if (!tests) {
    ...
    tests.forEach((test, i) => {
      if (test.constructor.name !== 'Object') {
        console.log(
          `%c-- tests[${i}] is not an Object`,
          'color:purple;font-weight:bold;'
        );
        return;
      } else if (typeof test.name !== 'string') {
        console.log(
          `%c-- tests[${i}].name is not a String`,
          'color:purple;font-weight:bold;'
        );
        return;
      } else if (!Array.isArray(test.args)) {
        console.log(
          `%c-- tests[${i}].args is not an Array`,
          'color:purple;font-weight:bold;'
        );
        return;
      } else if (!test.hasOwnProperty('expected')) {
        console.log(
          `%c-- tests[${i}].expected does not exist`,
          'color:purple;font-weight:bold;'
        );
        return;
      }
      ...
      if (result instanceof Error) {
        ...
        test.expected !== test.expected // is the expected value NaN?
